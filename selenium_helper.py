...
def init_webdriver(timeout=timeout):
...
def quit_scraper(driver, script_run_time):
    if driver:
        driver.quit()
    selenium_docker_ctrl('stop')
...
def safe_exec_selenium(func, driver, script_run_time, attempts=1, *args, **kwargs):
    ...
    while attempts > 0:
        ...
        try:
            ...
            attempts -= 1
            print(f'Unexpected Error. Retry? \n{e}')
...
def check_login_status(driver):
    ...
    if "mailbox/inbox" in driver.current_url:
        print("Login successful!")
        return
    elif EC.visibility_of_element_located((By.ID, "login_error"))(driver):
        raise MyLoginError
    else:
        raise Exception("Unexpected Login Error: Login Failed but Login Error Element not present.")

def login_iExWeb(driver):
    # opening the website in chrome.
    # print('Opening iExchangeWeb URL....')
    assert "iExchangeWeb" in driver.title, "not iExchangeWeb"
    username, password = get_credentials()
    loginBox = WebDriverWait(driver, timeout).until( \
